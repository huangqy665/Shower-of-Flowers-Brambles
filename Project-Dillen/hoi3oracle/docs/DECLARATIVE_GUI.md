...
```bash
cmake -S Project-Dillen -B Project-Dillen/build-win -A Win32
cmake --build Project-Dillen/build-win --config Debug --target gui_indexed_map_make
.\Project-Dillen\build-win\hoi3oracle\src\tools\Debug\gui_indexed_map_make.exe . GFX_example_map
```
